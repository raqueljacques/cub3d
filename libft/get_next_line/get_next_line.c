...
static char	*read_and_append(int fd, char *buffer, char *saved_text)
{
	...
	while (!gnl_strchr(saved_text, '\n') && bytes_read > 0)
	{
		...
		saved_text = gnl_strjoin(saved_text, buffer);
		free(temp_storage);
		if (!saved_text)
			return (NULL);
	}
...
static void	update_saved_text(char **saved_text, int newline_index)
{
	...
	length = gnl_strlen(*saved_text + newline_index + 1);
...
char	*get_next_line(int fd)
{
	...
	if (!buffer)
		return (NULL);
	if (!saved_text)
		saved_text = gnl_strdup("");
	saved_text = read_and_append(fd, buffer, saved_text);
	free(buffer);
	if (!saved_text)
		return (NULL);
	line = extract_next_line(&saved_text);
	if (!line && saved_text)
	{
		free(saved_text);
		saved_text = NULL;
